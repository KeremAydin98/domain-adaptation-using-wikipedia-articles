...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        attention_mask = input_ids.ne(self.tokenizer.pad_token_id).to(torch.float32)

        attention_masks = []
        for number in range(10):
            attention_masks.append((input_ids.eq(self.tokenizer.get_vocab()[str(number)]).to(torch.float32) * 1000)+ 1)

        for mask in attention_masks:
            attention_mask = attention_mask * mask

        attention_mask = attention_mask / attention_mask.max()
